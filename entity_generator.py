...
def spawn(entity_class, probability, mapdb, level, map_size=101, max_entities=None, max_entities_total=None,
    ...
    print(f"  - Map map_size: {map_size}x{map_size}")
...
def generate_additional_entity_data(entity_class, level):
    ...
    return {
        ...
        getattr(entity_class, attr, None) * level if attr in ['experience', 'armor', 'max_damage']
        else getattr(entity_class, attr, None)
        for attr in attributes if hasattr(entity_class, attr)
    }
